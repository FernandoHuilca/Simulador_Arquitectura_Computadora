...
public class MemoriaPrincipal {
    ...
    public String[] getDatosBinariosStrings() {
    String[] aux = new String[getNúmeroDeDatos()];
    for (int i = 0; i < getNúmeroDeDatos(); i++) {
        int valor = datos[i];
        String binario = SistemasNumericos.decimalABinarioDevuelveString(valor, bitsDeDatos);
        aux[i] = String.format("%s(%d)", binario, valor);
    }
    return aux;
}
